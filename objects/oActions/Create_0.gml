...
Unit_Attacks = function(unit_performing, unit_recieving) {
	if (unit_performing.side == SD.ALLIES)	Ally_Attacks(unit_performing, unit_recieving);
	if (unit_performing.side == SD.ENEMIES)	Eny_Attacks(unit_performing, unit_recieving);
}
...
Apply_Damage = function(unit_recieving, damage) {
	if (unit_recieving.side == SD.ALLIES)		unit_recieving.confidence	-= damage;	
	if (unit_recieving.side == SD.ENEMIES)	unit_recieving.hp			-= damage;	
...
Check_Element_Death = function(unit_recieving, tile_attacked) {
	if (unit_recieving.side == SD.ENEMIES)	Check_Eny_Death(unit_recieving, tile_attacked);
	if (unit_recieving.side == SD.ALLIES)		Check_Ally_Death(unit_recieving, tile_attacked);
}
